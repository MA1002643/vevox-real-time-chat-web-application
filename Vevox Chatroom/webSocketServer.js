...
const notifyClientsOfNewMessage = (
  creatorWebSocket,
  messageId,
  isQuestion,
  content,
  ...
  let newData = new messageEvents.ClientRecieveNewMessage(
    messageId,
    content,
    isQuestion,
    replyTo
  ...
  for ([sessionID, client] of Object.entries(webSocketSessions)) {
    if (client.webSocketConnection != creatorWebSocket)
      ...
      let customResponse = new messageEvents.ClientRecieveNewMessage(
        result.insertId,
        content,
        isQuestion,
        replyTo,
    ...
    ws.on("message", async function message(data) {
      ...
      try {
        switch (body["eventType"]) {
          case "message":
            [result, undef] = await connection.execute(
              "INSERT INTO Message SET message = ?, roomId = ?, replyMessageId = ?",
              [
                body["event"]["message"],
                body["event"]["roomId"],
                body["event"]["replyTo"] ?? null,
              ]
            );
            ...
            ws.send(
              JSON.stringify(
                new messageEvents.MessageAcknowledgement(
                  body["event"]["message"],
                  body["event"]["roomId"]
                )
              )
          ...
          case "getMessages":
            ...
            [rows, columns] = await connection.query(
              "SELECT messageId, replyMessageId, roomId, message, (questionId IS NOT NULL) AS isQuestion, Title, answerStatus, answerMessageID FROM `Message` LEFT JOIN `Questions` ON `Questions`.questionId = `Message`.messageId WHERE `roomId` = " +
                body["event"]["roomId"] // Only Loads messages for now for easy test/'mvp'
            );

            let getAllMessagesResponse =
              new messageEvents.GetAllMessagesResponse(rows);
          ...
          case "markAnswerQuestion":
            ...
            await connection.execute(
              "UPDATE Questions SET answerStatus = 1, answerMessageID = ? WHERE questionId = ?",
              [body["event"]["answerMessageId"], body["event"]["questionId"]]
            );

            let markAnswerQuestionResponse =
              new messageEvents.MarkQuestionAnswerResponse(
                body["event"]["questionId"],
                body["event"]["answerMessageId"]
              );
          ...
          case "createQuestion":
            ...
            [result, undef] = await connection.execute(
              "INSERT INTO Message SET message = ?, roomId = ?",
              [body["event"]["message"], body["event"]["roomId"]]
            );
            await connection.execute(
              "INSERT INTO Questions SET title = ?, questionId = ?",
              [body["event"]["title"], result.insertId]
            );

            ws.send(
              JSON.stringify(
                new messageEvents.QuestionAcknowledgement(
                  body["event"]["message"],
                  body["event"]["roomId"]
                )
              )
...
hasInitialised = new Promise(
  (resolve, reject) => (resolveInitializePromise = resolve)
);

initialize = async () => {
  try {
    // Build connection options and make SSL optional (use DB_SSL=true to enable)
    const connOptions = {
    ...
    if (process.env.DB_SSL === "true") {
      try {
        connOptions.ssl = { ca: fs.readFileSync("ca.pem") };
      } catch (e) {
        console.log(
          "Failed to read ca.pem for DB SSL, continuing without SSL:",
          e.message
        );
      }
    }
    ...
    console.log(
      "Falling back to a lightweight in-memory fake database for development."
    );

    // Minimal fake connection that mimics mysql2 promise interface used by this app.
    connection = {
      async execute(query, params) {
        // INSERT operations expect result.insertId
        if (/INSERT\s+INTO/i.test(query)) {
          return [{ insertId: Math.floor(Math.random() * 100000) }, undefined];
        }
  ...
  console.log("Database connected (real or fake). Starting websocket server");
  startWebSocketServer();
...
module.exports = {
  ...
  initializedPromise: hasInitialised,
};
